...
//    @PostMapping("/add") // BindingResult 사용하여 컨트롤러진입 이전 에러 해결
    public String addItem(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {

//        model.addAttribute("item", item); <- @ModelAttribute로 인해 item이 자동으로 들어감
...
//    @PostMapping("/add") // FieldError 생성자 활용하여 입력값 유지
    public String addItemV2(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        if(item.getPrice()!=null && item.getQuantity()!=null) {
            int rstPrice = item.getPrice() * item.getQuantity();
            if(rstPrice<10000) {
                bindingResult.addError(new ObjectError("item", null, null, "가격*수량의 합은 10,000원 이상이어야 합니다. 현재 값 = " + rstPrice));
            }
        }
...
//    @PostMapping("/add") // 오류 메세지 체계적으로 관리하기
    public String addItemV3(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        if(item.getQuantity()==null || item.getQuantity() >= 9999) {
            bindingResult.addError(new FieldError("item", "quantity", item.getQuantity(), false, new String[]{"max.item.quantity"}, new Object[]{"9,999"},  "수량은 최대 9,999까지 허용합니다."));
        }
        ...
        if(item.getPrice()!=null && item.getQuantity()!=null) {
            int rstPrice = item.getPrice() * item.getQuantity();
            if(rstPrice<10000) {
                bindingResult.addError(new ObjectError("item", new String[]{"totalPriceMin"}, new Object[]{"10,000", rstPrice}, null));
            }
        }
...
//            model.addAttribute("errors", errors);
    ...
    @PostMapping("/add") //@Validated, ItemValidator 이용
